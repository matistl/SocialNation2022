...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (parseInt(Cantidad) > EconomyGet[Item]) return SendError(`No tienes esa cantidad de items\nen tu balance.`, message)

        SendEmbed(`${Item.toUpperCase()}`, `Vendiste ${Minerales[Item].emote} **${parseInt(Cantidad).toLocaleString()}** \`${Item.toLowerCase()}\`\npor ${client.data.emotes.Money} **$${parseInt(Total).toLocaleString()}**.`, message)
