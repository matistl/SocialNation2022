...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		let Dinero = await ModelEconomy.findOne({ guild: message.guild.id, user: user.id });
		...
		const EmbedMoney = new Discord.MessageEmbed()
			.setAuthor('Balance', message.author.displayAvatarURL({ dynamic: true }))
			.addField(`${client.data.emotes.Dollar} ┇ **__Banco__**`, `${Dinero && Dinero.banco ? (Dinero.banco).toLocaleString() : 0}`)
			.addField(`${client.data.emotes.Money} ┇ **__Monedero__**`, `${Dinero && Dinero.monedero ? (Dinero.monedero).toLocaleString() : 0}`)
		...
		const EmbedMinerals = new Discord.MessageEmbed()
			.setAuthor('Minerales', message.author.displayAvatarURL({ dynamic: true }))
