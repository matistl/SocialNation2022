...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		if (!Color) return SendError('Especifica un color\npara apostar en ruleta.\n\n🔴 Red = \`x2\`\n⚫ Black = \`x3\`', message)

		if (!Cantidad) return SendError('Especifica una cantidad\npara apostar en ruleta.', message)
		...
		if (Color !== 'red' && Color !== 'black') return SendError('Elige un color válido.\nEntre \`red\` o \`black\`.', message);
		...
		if (Random === 'red') {
			if (Color === 'red') {
				...
				return SendEmbed('RULETA', `Has ganado ${client.data.emotes.Money} **$${(parseInt(Cantidad) * 2).toLocaleString()}**\npara tu monedero.`, message)
			...
			return SendError(`Has perdido ${client.data.emotes.Money} **$${parseInt(Cantidad).toLocaleString()}**\nde tu monedero.`, message)
		...
		if (Random === 'black') {
			if (Color === 'black') {
				...
				return SendEmbed('RULETA', `Has ganado ${client.data.emotes.Money} **$${(parseInt(Cantidad) * 3).toLocaleString()}**\npara tu monedero.`, message);
